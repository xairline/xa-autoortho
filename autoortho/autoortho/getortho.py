...
class Chunk(object):
    ...
    def __init__(self, col, row, maptype, zoom, priority=0, cache_dir='.cache'):
        ...
        self.cache_path = os.path.join(self.cache_dir, f"{self.col // 1000}_{self.row // 1000}", f"{self.chunk_id}.jpg")
    ...
    def get(self, idx=0, session=requests):
        ...
        MATRIXSET = "g"
        MAPTYPES = {
            "EOX": f"http://tiles.maps.eox.at/wmts/1.0.0/s2cloudless-2023/default/WGS84/{self.zoom}/{self.row}/{self.col}.jpg",
...
class Tile(object):
    ...
    def _find_cache_file(self):
        ...
        with self.tile_lock:
            for z in range(self.zoom, (self.min_zoom - 1), -1):
                cache_file = os.path.join(self.cache_dir, f"{self.col // 1000}_{self.row // 1000}",
                                          f"{self.row}_{self.col}_{self.maptype}_{self.zoom}_{z}.dds")
    ...
    def write(self):
        outfile = os.path.join(self.cache_dir, f"{self.col // 1000}_{self.row // 1000}",
                               f"{self.row}_{self.col}_{self.maptype}_{self.zoom}_{self.zoom}.dds")
        self.ready.clear()
